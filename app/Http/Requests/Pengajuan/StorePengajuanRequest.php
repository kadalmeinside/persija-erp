...
class StorePengajuanRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'attachment' => 'nullable|required_if:tipe_pengajuan,'.PengajuanType::LANGSUNG->value.'|file|mimes:pdf,jpg,png,jpeg|max:2048',
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Conditional Logic for Beneficiary
            if (isset($data['metode_pembayaran']) && $data['metode_pembayaran'] === PaymentMethod::TRANSFER->value) {
                if (isset($data['tipe_pengajuan']) && $data['tipe_pengajuan'] === PengajuanType::LANGSUNG->value) {
                    // Check sub_tipe_penerima from input if available, or infer
                    $isKaryawan = $this->input('sub_tipe_penerima') === 'Karyawan';
