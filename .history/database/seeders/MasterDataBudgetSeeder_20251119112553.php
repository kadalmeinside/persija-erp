...
class MasterDataBudgetSeeder extends Seeder
{
    ...
    ): array
    {
        ...
        if ($isGlPattern) {
            ...
            // Validasi State Departemen
            if (!$currentDepartemenId) {
                $dept = Departemen::firstOrCreate(['nama_departemen' => 'UMUM']);
                $currentDepartemenId = $dept->id;
            }

            // Validasi State Program Kerja (MODIFIED)
            // Jika tidak ada program kerja yang aktif sebelumnya, kita LEMPAR ERROR.
            if (!$currentProgramId) {
                $this->command->error("  [ERROR] Baris {$rowNumber}: SKIP. Akun GL '{$kodeAkun}' - '{$uraian}' tidak memiliki Program Kerja Induk.");
                // Kembalikan state tanpa perubahan agar tidak merusak baris berikutnya (jika ada)
                return compact('currentDepartemenId', 'currentProgramId', 'lastKodeAkunPrefix');
            }
