...
class MasterDataBudgetSeeder extends Seeder
{
        ...
        DB::transaction(function () use ($absolutePath) {
            ...
            if (($handle = fopen($absolutePath, "r")) !== false) {
                ...
                while (($row = fgetcsv($handle, 0, ';')) !== false) {
                    ...
                    $currentDepartemenId = $result['current_departemen_id'];
                    $currentProgramId = $result['current_program_id'];
                    $lastKodeAkunPrefix = $result['last_kode_akun_prefix'];
    ...
    ): array
    {
        ...
        if (in_array(strtoupper($kodeAkun), ['BIAYA', 'PENDAPATAN', 'KODE AKUN', 'ANGGARAN TAHUN 2025'])) {
        ...
        if ($isDepartemen) {
            ...
            // Perlu membersihkan nama departemen yang mungkin berada di kolom A
