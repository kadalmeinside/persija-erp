...
class MasterDataBudgetSeeder extends Seeder
{
    ...
    private $fileName = "anggaran2.csv";
        ...
        DB::transaction(function () use ($absolutePath) {
            ...
            if (($handle = fopen($absolutePath, "r")) !== false) {
                ...
                while (($row = fgetcsv($handle, 0, ';')) !== false) {
                    ...
                    // --- Pemeriksaan keamanan dan fallback ---
                    if (!is_array($result)) {
    ...
    {
        // Ambil dan bersihkan data dari karakter tersembunyi dan spasi berlebih
        $kodeAkun = trim($row[0] ?? '');
        $uraian = trim($row[1] ?? '');
        
        // **FIX KRITIS: Sanitasi string untuk mengatasi karakter non-cetak dari CSV/Excel**
        // Menghapus semua karakter kontrol ASCII dan non-breaking space (\xc2\xa0)
        $kodeAkun = preg_replace('/[\x00-\x1F\x7F\xc2\xa0]/', '', $kodeAkun);
        $uraian = preg_replace('/[\x00-\x1F\x7F\xc2\xa0]/', '', $uraian);

        // Trim ulang setelah sanitasi
        ...
        if ($isDepartemen) {
            // Kita sudah sanitasi, jadi ambil sisanya dari uraian (kolom B) atau kode akun (kolom A)
        ...
        // Kriteria: Bukan Akun GL LENGKAP, Punya Anggaran 0, dan ada Uraian
        ...
        if ($isAkun) {
            ...
            // Pastikan Program ada (FALLBACK ke 'Operasional Umum' jika Program Header terlewat)
            if (!$currentProgramId) {
