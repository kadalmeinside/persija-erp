...
class PengajuanController extends Controller
{
     ...
     * Digunakan untuk Cascading Dropdown level 1 di form Create.
     */
    private function getActivePeriodeId()
    {
        // Prioritaskan periode aktif flag
        $periode = PeriodeAnggaran::where('is_active', true)->first();
        
        if (!$periode) {
            // Fallback: Periode tahun berjalan
            $now = Carbon::now();
    ...
    public function getProgramsByDepartemen(Request $request)
    {
        ...
        // Query menggunakan id_periode_anggaran
        $programs = BudgetMaster::where('id_departemen', $request->id_departemen)
                    ->where('id_periode_anggaran', $periodeId)
                    ->with('programKerja')
                    ->get()
                    ->pluck('programKerja')
                    ->unique('id')
