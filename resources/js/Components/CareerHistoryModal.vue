...
const submit = () => {
    form.post(route('admin.karyawan.riwayat-karir.store', props.karyawanId), {
        forceFormData: true,
        preserveScroll: true,
        onSuccess: () => {
            emit('saved');
    ...
    <Modal :show="show" @close="close" maxWidth="2xl">
        <div class="p-6">
            ...
            <form @submit.prevent="submit" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <InputLabel value="Departemen Baru" />
                        <select v-model="form.id_departemen" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="">-- Sama dengan sebelumnya --</option>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <InputLabel value="Gaji Pokok Baru (Rp)" />
