...
const props = defineProps({
    ...
    masterProgram: Array,
    masterAkun: Array,
    laporan: Object // Data laporan yang sudah ada
});
...
// Pre-fill form dengan data laporan yang ada
const form = useForm({
    tgl_laporan: props.laporan?.tgl_laporan || new Date().toISOString().split('T')[0],
    items: props.laporan?.detail.map(item => ({
        deskripsi_bon: item.deskripsi_bon,
        nominal_bon: parseFloat(item.nominal_bon),
        id_program: item.id_program_beban,
        id_akun: item.id_akun_beban,
        file_bukti: null, // Reset file input
        existing_path: item.bukti_path // Simpan path lama
    })) || [],
    ...
    try {
        const response = await axios.get(route('admin.pengajuan.getBudgetBalance'), {
            params: { 
                ...
                tgl_pengajuan: form.tgl_laporan 
            }
        });
...
const saveItem = () => {
    ...
    // Validasi file_bukti wajib diupload jika tidak ada existing_path
    if (!modalForm.value.file_bukti && !modalForm.value.existing_path) {
...
const submit = () => {
    ...
    const missingProofIndex = form.items.findIndex(item => !item.file_bukti && !item.existing_path);
    if (missingProofIndex !== -1) {
        alert(`Rincian baris ke-${missingProofIndex + 1} belum melampirkan bukti bon/struk.`);
        return;
    }

    if (selisih.value > 0 && !form.bukti_pengembalian && !props.laporan?.bukti_pengembalian_path) {
        alert("Terdapat sisa uang. Mohon lampirkan bukti transfer pengembalian."); return;
    }
    // Gunakan POST dengan _method: PUT tidak diperlukan di Inertia form helper jika route support PUT, 
    // tapi karena ada file upload, Inertia otomatis pakai FormData. 
    // Laravel resource update biasanya butuh _method: PUT/PATCH jika FormData.
    // Tapi kita pakai custom route 'update' (POST) di controller untuk simplifikasi file upload.
    // Cek route di web.php nanti. Kita pakai POST ke endpoint update.
    form.post(route('admin.settlement.update', props.pengajuan.id), {
        preserveScroll: true,
        onSuccess: () => alert('Revisi Laporan berhasil dikirim!'),
    });
};
</script>

<template>
    <Head title="Revisi Laporan Pertanggungjawaban" />
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">Revisi Laporan Pertanggungjawaban</h2>
            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <template v-else>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        <div class="p-6 bg-gradient-to-r from-pink-50 via-white to-white flex flex-col md:flex-row justify-between items-center gap-4">
                            <div class="flex items-center gap-4">
                                <div class="p-3 bg-pink-100 text-pink-600 rounded-full">
                                    <PencilSquareIcon class="w-8 h-8"/>
                                </div>
                                <div>
                                    <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Mode Revisi</p>
                            ...
                            <div class="text-right bg-white p-4 rounded border border-pink-100 shadow-sm">
                                <p class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Total Diterima</p>
                                <p class="text-3xl font-bold text-pink-700">{{ formatCurrency(totalUangMuka) }}</p>
                            </div>
                        </div>
                        <!-- Catatan Revisi -->
                        <div class="px-6 py-3 bg-pink-50 border-t border-pink-100 text-sm text-pink-800 italic">
                            <b>Catatan Revisi:</b> "{{ pengajuan.catatan_header }}"
                        </div>
                    </div>
                    ...
                    <form @submit.prevent="submit">
                        ...
                        <div class="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden mt-6">
                            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-0.5">Perbaiki rincian belanja sesuai kebutuhan.</p>
                                </div>
                                <div class="flex items-center gap-3">
                                        ...
                                        <tbody class="divide-y divide-gray-100">
                                            ...
                                            <tr v-for="(item, index) in form.items" :key="index" class="hover:bg-gray-50">
                                                ...
                                                <td class="px-4 py-3 text-center">
                                                    <div v-if="item.file_bukti" class="flex items-center justify-center text-green-600 text-xs" title="File Baru">
                                                        <PaperClipIcon class="w-4 h-4 mr-1"/> Baru
                                                    </div>
                                                    <div v-else-if="item.existing_path" class="flex items-center justify-center text-gray-600 text-xs" title="File Lama">
                                                        <PaperClipIcon class="w-4 h-4 mr-1"/> Lama
                                                    </div>
                            ...
                            <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col justify-center">
                                <div v-if="selisih > 0">
                                    ...
                                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:bg-gray-50 transition cursor-pointer relative group">
                                        ...
                                        <div v-if="!form.bukti_pengembalian && !props.laporan?.bukti_pengembalian_path" class="flex flex-col items-center">
                                            ...
                                            <p class="text-sm text-gray-600 font-medium">Klik untuk upload bukti baru</p>
                                        </div>
                                        <div v-else class="flex flex-col items-center">
                                            <div class="p-2 bg-green-100 rounded-full mb-2"><CheckIcon class="w-6 h-6 text-green-600"/></div>
                                            <p class="text-sm text-green-700 font-bold truncate max-w-[200px]">
                                                {{ form.bukti_pengembalian ? form.bukti_pengembalian.name : 'File Lama Tersimpan' }}
                                            </p>
                        ...
                        <div class="mt-8 flex justify-end pt-6 border-t border-gray-200">
                            <PrimaryButton :class="{ 'opacity-50': form.processing }" :disabled="form.processing" class="px-6 py-3 text-base shadow-lg bg-pink-600 hover:bg-pink-700 border-pink-600">
                                <span v-if="form.processing">Menyimpan...</span>
                                <span v-else>Kirim Revisi</span>
                            </PrimaryButton>
                        </div>
                    </form>
                </template>
                        ...
                        <div class="mt-1 flex items-center justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:bg-gray-50 transition cursor-pointer relative group">
                            ...
                            <div class="space-y-1 text-center">
                                ...
                                <div class="flex text-sm text-gray-600 justify-center">
                                    ...
                                    <span v-else-if="modalForm.existing_path" class="font-medium text-gray-600">File Lama Tersimpan</span>
                                    <span v-else class="font-medium text-indigo-600 hover:text-indigo-500">Upload file</span>
                                </div>
                                <p v-if="!modalForm.file_bukti && !modalForm.existing_path" class="text-xs text-gray-500">PNG, JPG, PDF up to 2MB</p>
                                <p v-else class="text-xs text-green-600 font-bold">File siap dikirim</p>
                            </div>
                        </div>
                    </div>
                </div>
