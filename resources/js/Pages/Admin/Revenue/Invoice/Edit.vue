...
const form = useForm({
    id_pelanggan: props.invoice.id_pelanggan,
    id_departemen: props.invoice.id_departemen,
    tgl_invoice: props.invoice.tgl_invoice ? String(props.invoice.tgl_invoice).substring(0, 10) : '',
    tgl_jatuh_tempo: props.invoice.tgl_jatuh_tempo ? String(props.invoice.tgl_jatuh_tempo).substring(0, 10) : '',
    catatan: props.invoice.catatan,
    items: props.invoice.detail.map(item => ({
        deskripsi_item: item.deskripsi_item,
        kuantitas: item.kuantitas,
        harga_satuan: item.harga_satuan,
        id_akun_pendapatan: item.id_akun_pendapatan
    })),
    id_tax_ppn: props.invoice.id_tax_ppn,
    id_tax_pph: props.invoice.id_tax_pph
});
...
const submit = () => {
    if (confirm('Mengubah invoice akan menghapus jurnal lama dan membuat jurnal baru. Lanjutkan?')) {
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Invoice: {{ invoice.nomor_invoice }}</h2>
        </template>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    ...
                    <div class="mb-6 p-4 bg-yellow-50 border-l-4 border-yellow-400 text-yellow-700">
                        ...
                        <p>Mengedit invoice akan merevisi jurnal akuntansi dan realisasi anggaran secara otomatis.</p>
                    </div>
